...
TRIGGER_PIN = 17  # GPIO pin for trigger
...
PERSON_DETECTION_TIMEOUT = 10  # 10 seconds timeout
OBSTACLE_DISTANCE_THRESHOLD = 0.3  # 30 cm obstacle detection threshold
...
# Navigation without person detection
def navigate_without_person():
    """
    Navigation algorithm when no person is detected
    """
    global no_person_start_time
    
    while True:
        # Check if still no person detected
        if no_person_start_time is None or time.time() - no_person_start_time < PERSON_DETECTION_TIMEOUT:
            move_forward()
            ...
            # Reset and stop when person is found
            stop_motor()
            break
    ...
    while True:
        ...
        if person_count > 0:
            ...
            if not person_detected:
                # Start tracking no person time if not already tracking
                if no_person_start_time is None:
                    no_person_start_time = time.time()
                
                # If no person detected for more than 10 seconds, start navigation
                if time.time() - no_person_start_time >= PERSON_DETECTION_TIMEOUT:
                    navigate_without_person()
                    person_detected = False

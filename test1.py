...
def navigate():
    ...
    while True: # run loop forever
        ...
        # it returns true when obstacle is near (less than the threshold value)
            ...
            ## Always turn left first (consistent wall-following behavior)
            print("[INFO] : Turning left 90 degrees...")
            rotate_left(5)
            CURRENT_HEADING = (CURRENT_HEADING + (360 - 90)) % 360 # updating the heading to left
            print(f"[INFO] : Done turning left 90 degrees. New heading: {CURRENT_HEADING}")
            ...
            # if obstacle is still there, then turn left again (180 degrees total from original direction)
            print("[WARN] : Obstacle still detected, turning left another 90 degrees...")
            rotate_left(5)
            CURRENT_HEADING = (CURRENT_HEADING + (360 - 90)) % 360 # turning left again
            print(f"[INFO] : Done turning left (180 degrees total). New heading: {CURRENT_HEADING}")

            # check again whether any obstacle is at the front
            if not check_obstacle(): # will return false if no obstacle is found
                continue    # continue to next iteration if no obstacle is found

            # if obstacle is still there, then turn left again (270 degrees total from original direction)
            print("[WARN] : Obstacle still detected, turning left another 90 degrees...")
            rotate_left(5)
            CURRENT_HEADING = (CURRENT_HEADING + (360 - 90)) % 360
            print(f"[INFO] : Done turning left (270 degrees total). New heading: {CURRENT_HEADING}")

            # At this point, if there's still an obstacle, the robot is completely surrounded
            # Turn left one more time to complete the full circle and try again
            if check_obstacle():
                print("[ERROR] : Robot appears to be completely surrounded!")
                print("[INFO] : Completing full rotation and trying again...")
                rotate_left(5)
                CURRENT_HEADING = (CURRENT_HEADING + (360 - 90)) % 360
                print(f"[INFO] : Completed full rotation. New heading: {CURRENT_HEADING}")
